...
const SCHEMA = [
  ...
  { key: 'N_obstacles', label: 'Obstacles (N)', min: 1, max: 15, step: 1 },
  { key: 'visibility_range', label: 'Visibility range (m)', min: 1.0, max: 5.0, step: 0.5 },
  { key: 'K_visible', label: 'Visible obstacles (K)', min: 1, max: 5, step: 1 },
  { key: 'obstacle_drift', label: 'Obstacle drift', min: 0.0, max: 0.1, step: 0.005 },
]
...
export default function Room5_Obstacles() {
  ...
  const [obstaclesNow, setObstaclesNow] = useState([])
  ...
  const handleMessage = useCallback((msg) => {
    if (msg.type === 'room_info') {
      ...
      if (msg.obstacles) setObstaclesNow(msg.obstacles)
      if (msg.loss != null) {
  ...
  const onReplayStep = useCallback(
    (step) => {
      ...
      setObstaclesNow(point.obstacles || [])
      if (prev) {
  ...
  return (
    <div style={styles.layout}>
      <aside style={styles.sidebar}>
        ...
        <button
          style={{ ...styles.genButton, ...(status !== 'complete' ? styles.genButtonDisabled : {}) }}
